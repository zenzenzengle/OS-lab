...
{  
  ...
  // 这里不能映射CLINT，因为PLIC中保存了内核的最低虚拟地址,xv6的正常工作依赖于用户的最高虚拟地址小于PLIC
...
walk(pagetable_t pagetable, uint64 va, int alloc)
{
  ...
  for(int level = 2; level > 0; level--) {// 划分为三级
...
kvmpa(uint64 va)
{
  ...
  pte = walk(myproc()->kernel_pagetable, va, 0);   // 修改为使用进程自己的内核页表
  if(pte == 0)
    panic("kvmpa");
  if((*pte & PTE_V) == 0)
    panic("kvmpa");
